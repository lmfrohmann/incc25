...
| **Metric** | RMSE (60% of final score) + methodology write-up (40%). |
...
1. **Residual demand is king.** The single feature `es_demand - es_wind - es_solar` explains ~64% of variance. Pumped storage acts as a flexible balancing resource.
2. **Strong diurnal regime.** Spain pumps during solar hours (H10-15) and generates during evening peak (H18-21). Regime-switching models capture this asymmetry.
3. **Rolling features help.** 24h/168h rolling mean, std, and momentum of key fundamentals add meaningful signal, especially residual demand momentum, which captures trend.
4. **Econometric diversity pays.** Even weaker individual econometric models (RMSE ~800-860) improve ML ensembles when blended, because they make structurally different errors.
5. **Per-hour bias correction matters.** Expanding-window bias correction (per-hour for CatBoost, per-hour for econometric) consistently improves RMSE.
6. **Capacity constraints bind.** Clipping predictions to available production/pumping capacity (from unavailability data) provides a free ~2-point RMSE improvement.
